# This is the function for the transcript to email id progression part.
# It takes the audio's transcript, the file name that contains the list of email ids that were actually said in the audio.
# It takes a third parameter which is the name of the output file (csv) (add .csv to the name while passing it. ) where it will store the results.
def take_transcript_and_do_all_from_there(transcript, actual_list_file_name_with_suffix, output_file_name):
    ## imports
    import re
    import pandas as pd
    import ast
    ...
    def return_context(transcript, extracted_email):
        ans = llm.invoke(
            "Go through the following conversation transcript. :" + transcript + "Find the the sentence or sentences from the transcript where the following email id was said." + extracted_email + "Now, check if the email id shown to you which was extracted from the transcript matches with the transcript?")
        return ans.content
    ##################################################################################
    ##################################################################################
    def is_email_legible_llm_version(transcript, email_id):
        ...
        # Prompt Method1
        answer1 = llm.invoke("Given the following call transcript, return if the email seems to be correctly extracted or if the user must be asked again to clarify their email. Here is the transcript: " + transcript + "This is the inferred email: " + email_id + "Now, if you think email id is correctly extracted and there seems to be no room for any discrepency, return your answer as a singular word, 'True'. If you think user needs to be asked again to clarify what the said, return a singular word, 'False'.")

        # Prompt Method2
        ans2_1 = llm.invoke(
            "Go through the following conversation transcript. :" + transcript + "Find the the sentence or sentences from the transcript where the following email id was said." + email_id + "Now, check if the email id shown to you which was extracted from the transcript matches with the transcript?")
        ans2_2 = llm.invoke(
            "Read the following response generated after comparison between the email id extracted from the transcript and the email itself: '" + ans2_1.content + "' Now return a singular word 'True' if the response says that it is matching. If the response says that it is not mathcing, return the singular word 'False'. Please answer exactly as stipulated.")

        # Prompt Method3
        p3_1 = """Go through the following conversation transcript. : '"""
        p3_2 = """' Now look at the following email id which has been extracted from this conversational transcript: '"""
        p3_3 = """Now, return a single word 'True' if you think the email has been correctly exracted or a singular word 'False' if it has been incorrectly extracted."""
        ans3 = llm.invoke(p3_1 + transcript + p3_2 + email_id + p3_3)

        # Prompt Method4
        p4_1 = """Go through the following conversation transcript. : '"""
        p4_2 = """' Now look at the following email id which has been extracted from this conversational transcript: '"""
        p4_3 = """Now, find the exact line in the transcript from ehich this email id was extracted. Return this line s it is. Do not give any extra text surrounding it in your output. Make sure that you return only the specific sentext in the transcript as it is."""
        ans4_1 = llm.invoke(p4_1 + transcript + p4_2 + email_id + p4_3)
        p4_4 = ans4_1.content
        print(p4_4)
        p4_5 = """The following is the line in a given transcript : '"""
        p4_6 = """' This is the email that has been extracted : '"""
        p4_7 = """Now, return a single word 'True' if you think the email has been correctly exracted or a singular word 'False' if it has been incorrectly extracted."""
        ans4_2 = llm.invoke(p4_5 + p4_4 + p4_6 + p4_7)

        # return answer1.content
        # return ans2_2.content
        return ans3.content
        # return ans4_2.content
    ##################################################################################
    ##################################################################################
    ...
    # Calling environment
    # Actual list:
    with open(actual_list_file_name_with_suffix, 'r') as file:
    ...
    for i in range(len(first_mail_list)):
        chosen_mail = choose_best_using_llm_call_for_each_singular_email_for_atr_transform(first_mail_list[i],
                                                                                           atr_changed_mails[i])
    ...
    df_ans = pd.DataFrame(email_progression)
    df_ans.to_csv(output_file_name, index=False)
    #
    return df_ans
    ...
    # return email_progression



# transcript = """[00:00.000 --> 00:05.000]  The email ids with my teammates are as follows
# [00:05.000 --> 00:09.000]  JohnDoe123 at the rate gmail dot com
# [00:09.000 --> 00:12.000]  Sarah Smith 456 at the rate yahoo dot com
# [00:12.000 --> 00:16.000]  Mike Brown 789 at the rate hotmail dot com
# [00:16.000 --> 00:20.000]  Emily Jones 101 at the rate gmail dot com
# [00:20.000 --> 00:23.000]  David Wilson triple 2 at the rate yahoo dot com
# [00:23.000 --> 00:27.000]  Lisa Jackson triple 3 at the rate hotmail dot com
# [00:27.000 --> 00:31.000]  Chris Roberts triple 4 at the rate gmail dot com
# [00:31.000 --> 00:34.000]  Laura White triple 5 at the rate yahoo dot com
# [00:34.000 --> 00:37.000]  Kevin Miller triple 6 at the rate hotmail dot com
# [00:37.000 --> 00:41.000]  Amanda Jones triple 7 at the rate gmail dot com
# [00:41.000 --> 00:46.000]  Brian Wilson triple 8 at the rate yahoo dot com
# [00:46.000 --> 00:50.000]  Jessica Smith triple 9 at the rate yahoo dot com
# [00:50.000 --> 00:54.000]  Matthew Brown 1010 at the rate gmail dot com
# [00:57.000 --> 01:07.000]  Thanks for watching!"""

# transcript = """The following are the email IDs of my team.
# Alia Khan 174 at therategmail.com
# Yasmin Ahmed 530 at therateyahoo.com
# Mohammad Alia 661 at theratehotmail.com
# Priya Sharma 00 at therategmail.com
# Ananya Singh 109 at therateyahoo.com
# Rf at a 1215 at theratehotmail.com
# Yishan Gupta 777 at therategmail.com
# Aishwarya Rao, triple 3 at therateyahoo.com
# Arjun Mahata, triple 4 at theratehotmail.com
# Neha Patel, triple 5 at therategmail.com
# Vishal Kumar, triple 6 at therateyahoo.com
# Priyanka Singh, triple 7 at theratehotmail.com
# Aditya Shah, triple 8 at therategmail.com
# Meenakshi Singh, triple 2 at therateyahoo.com
# Rajesh Mahata, 456 at theratehotmail.com
# Anushka Sharma, 789 at therategmail.com
# Devendra Singh, triple 5 at therateyahoo.com
# Nandini Gupta, triple 7 at theratehotmail.com
# Rajiv Kumar, triple 9 at therategmail.com
# Priyanka Valia, 1, 2, 3 at therateyahoo.com
# Rahul Sharma, 4, 5, 6 at theratehotmail.com
# Ananya Singh, 789 at therategmail.com
# Vikasmit, triple 5 at therateyahoo.com
# Priyanka Gupta, triple 7 at theratehotmail.com
# Amit Joshi, triple 9 at therategmail.com
# Priyanka Mahata, 123 at theratehotmail.com
# Rishap Jain, 456 at theratehotmail.com
# Rohi Singh, 789 at therategmail.com
# Akash Masha, triple 5 at therateyahoo.com
# Priyanka Vatal, triple 7 at theratehotmail.com
# These are the email IDs of my team.
# """

transcript = """Hello, the following are the email IDs of my team, Maria Gonzalez 82 at the rate mail.com,
Jake Davis 1995 at the rate outlook.com, Lee Way, Lee Dot Way underscore 2021 at the rate
Yahoo.co.uk, Fazal.com underscore 786 at the rate Hotmail.com, Sophie Dot Martin 123 at
the rate lift.com, John underscore doh 99 at the rate ahole.com, Natalie Dot Fernandez dot 87
at the rate female dot com, Alexander Dot Smith underscore 55 at the rate iCloud.com,
CHIA RA dot R O double S I underscore it earlier at the rate mailbox dot org, Yuki underscore
Takahashi Dot 2022 at the rate prodom mail dot com.
"""

val = take_transcript_and_do_all_from_there(transcript, "/Users/adityaagre/Desktop/Vibtree_in_office/Email_Extraction/Hugging_sound_fine_tune_for_email/dreji18_repo/Audio_data/Random_tests/Test_1/Test_1_actual.txt", "/Users/adityaagre/Desktop/Vibtree_in_office/Email_Extraction/Hugging_sound_fine_tune_for_email/dreji18_repo/Audio_data/Random_tests/Test_1/Test_1_result_with_whisper.csv")
