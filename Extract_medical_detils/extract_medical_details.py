def extract_details(transcript, directions_list):
    ##imports
    import re
    import pandas as pd
    import ast
    ...
    prompt_answer = """Look at the following Transcript : {} 
    Now consider the following question and answer based on the transcript: {}"""
    prompt_mcq = """Look at the following Transcript : {} 
    Now consider the following question : {} 
    Look ate the following options and return any one as your answer. Only return the option text nothing else. {} Note: Only return the option you pick. Do not answer ina sentence."""

    for direction in directions_list:
        name = direction['name']
        # print(type(name))
        if(direction['Type'] == "Answer"):
            prompt_local = prompt_answer.format(transcript, direction['Question'])
            answer = llm.invoke(prompt_local)
            answer = answer.content
            # print(answer)
            return_dictionary[name] = answer
            print(answer)
            # print(return_dictionary)
        elif(direction['Type'] == "MCQ"):
            prompt_local = prompt_answer.format(transcript, direction['Question'], direction['Options'])
            answer = llm.invoke(prompt_local)
            answer = answer.content
            # print(answer)
            return_dictionary[name] = answer
...
# [
#     {    'name' : "Full Name",
#         'Question' : "What is the customers's name?
#             'Type' : "MCQ" or "Answer"
#             'Options:' "['dddddddddddddddd']"
#     }
# ]
...
directions = [
    {
    'name' : "Full Name",
    'Question' : "Please extract the customer's full name. Return only the customer's name. Nothing else.",
        'Type' : "Answer"
    },
    {
    'name' : "Address",
    'Question' : "Please extract the customer's address. Return only the customer's address. Nothing else. In case the address is not mentioned, return the single word 'None'",
        'Type' : "Answer"
    },
    {
    'name' : "Phone Number of Customer",
    'Question' : "Please extract the customer's phone number. Return only the customer's Phone number. Nothing else. In case the Phone number is not mentioned, return the single word 'None'",
        'Type' : "Answer"
    },
    {
        'name': "Email ID of Customer",
        'Question': "Please extract the customer's Email ID. Return only the customer's Email ID. Nothing else. Do not answer in a sentence. Only return the email ID. In case the emai ID is not mentioned, return the single word 'None'",
        'Type': "Answer"
    },
    # {
    #     'name': "Email ID of Customer",
    #     'Question': "Please extract the customer's Email ID. Return only the customer's Email ID. Nothing else.",
    #     'Type': "Answer"
    # },
    {
        'name': "POA?",
        'Question': "Has the customer's pewer of attorney been given to someone else? Pick the closest option. Only return the option you pick. Do not answer in a sentence.",
        'Type': "MCQ",
        'Options': "['Yes', 'No', 'Cannot be determined']"
    },
    {
        'name': "POA - Relation. ",
        'Question': "In case the Power of Attorney of the customer has been given to someone else, return their relation. (Eg: Son or Daughter or Spouse, etc.). In case the power of attorney is with the customer themself, return a single word 'None.' Nothing else.",
        'Type': "Answer"
    },

    {
        'name': "New Medical Pendant OR Existing?",
        'Question': "Does the customer have an existing medical pendant? Pick the closest option. Only return the option you pick. Do not answer in a sentence.",
        'Type': "MCQ",
        'Options': "['Yes', 'No', 'Cannot be determined']"
    },
    {
        'name': "How much paying per month?",
        'Question': "If the customer has an existing Pendant, return the amount they are paying per month. Return just the amount. Nothing else. In case they are not having an existing pendant, return '0'",
        'Type': "Answer"
    },

    {
        'name': "Is the customer in contract",
        'Question': "Is the customer in a contract? Pick the closest option. Only return the option you pick. Do not answer in a sentence.",
        'Type': "MCQ",
        'Options': "['Yes', 'No', 'Cannot be determined']"
    },
    {
        'name': "Does the Customer Qualify?",
        'Question': "Does the have Qualify? Pick the closest option. Only return the option you pick. Do not answer in a sentence.",
        'Type': "MCQ",
        'Options': "['Yes', 'No', 'Cannot be determined']"
    },
    {
        'name': "Processing Date",
        'Question': "If the processing date is mentioned, return it in the DD/MM/YYYY format. Return just the date. Nothing else. In case it is not mentioned, return 'None'",
        'Type': "Answer"
    },
    {
        'name': "Recurring Date",
        'Question': "If the Recurring Date is mentioned, return it in the DD/MM/YYYY format. Return just the date. Nothing else. In case it is not mentioned, return 'None'",
        'Type': "Answer"
    },

    {
        'name': "Other Promos?",
        'Question': "Which other Promos are there? Pick the closest option. Only return the option you pick. Do not answer in a sentence.",
        'Type': "MCQ",
        'Options': "['Home Security', 'Solar', 'Home Insurance', 'At&T Cellular', 'None']"
    },
...
print(extract_details(transcript, directions))